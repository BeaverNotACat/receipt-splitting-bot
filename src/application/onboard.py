...
from src.domain.value_objects import ChatID, ReceiptID  # noqa: TC001
...
class OnboardUser(Interactor[OnboardUserDTO, None]):
    ...
    async def __call__(self, context: OnboardUserDTO) -> None:
        user = await self.register_user(context.chat_id, context.name)
        if context.receipt_id is not _sentinel:
            await self.add_to_receipt(user, context.receipt_id)

    async def register_user(self, chat_id: ChatID, name: str) -> RealUser:
        user = self.user_service.create_real_user(chat_id, name)
